...
Habituals are a particularly amenable domain to manipulating the target probability that a speaker aims to communicate, as this is expressed in an easily-interpretable frequency with which a person does an action (e.g., "Last month, Mary smoked cigarettes 3 times.").^[
...
In addition to directly manipulating the target propensity, these experiments provide the first test of the generality of our theory, that it can apply to both categories and events.
...
Two possibilities for what $h$ could relfect are: the actual, past frequency of the event (e.g., how often Mary has actually smoked in the past week) or the predictive probability that this event will occur in the near future (e.g., how likely it is that Mary will smoke next week)? 
In Expts. 3a \& 3b, we show that $h$, the target of communication, is a speaker’s prediction about the future frequency of action, and not past frequency.
...
4. Birds lay eggs and are female.
...
(3) and (4) show subtle interactions of generic language with another linguistic device: conjunction. 
(3) sounds true but why it is true is not obvious.
We don't think there to be international elephants (i.e., individual elephants who live in Africa part time and Asia part time); thus, the statement can only be glossed as *some elephants live in Africa* and *some elephants live in Asia*, and perhaps taken collectively, *most elephants live in either Africa or Asia* [@Nickel2008]. 
This reading of (3) gives it the same semantic content as two statements: "Elephants live in Africa. Elephants live in Asia."
Our model is able to flexibly accomodate this statement by having a distribution over possible thresholds. 
...
(4) highlights that simple models of conjunction are not sufficient for generic language.
If we take the conjunction to imply the conjunctive property (i.e., *lays eggs and is female*), then our model would incorrectly endorse this statement, which is judged intuitively to be false [@Leslie2007].

(5) is used to argue that even if the property is widespread (perhaps in a soft sense, as in our model, relative to other kinds or groups), we might find it avervise to actually assert the generic.
