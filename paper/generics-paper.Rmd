...
The fact that these generalizations do not reduce to particular quantitative statements implies that what makes them true or false (in terms of precise quantitative measurements) is also underspecified [@Carlson1977; @Leslie2008; @NickelBlackwell].

Yet, listeners still have intuitions about how strongly a generalization should hold.
...
We can treat the problem as a problem of Bayesian inference about an unknown latent cause over the data we observe.
Suppose we observe a feature $F$ among instances of a kind $K = \{k_1, k_2, ...\}$: What can we infer about the probability of that feature in future instances of $K$?

Bayes' rule provides a rational framework for answering such problems.
Suppose *a priori* we have no informative beliefs about the prevalence of the feature in the kind (i.e., $P(F \mid K) \sim \text{Uniform}(0, 1) = \text{Beta(1, 1)}$).
...
For example, consider the prevalence of \textsc{being friendly} (F) among \textsc{dogs} (K); call this $p$.
We may encounter some individual dogs, note how many of them were friendly, and form a posterior belief distribution about the friendliness of dogs.
...
Thus, it is not surprising that language provides a simple way to communicate these generalizations to one another.
