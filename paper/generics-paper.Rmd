...
rs.glm <- glm(response ~ prevalence, 
              data = d.tj.w.prev.cue, family = 'binomial')

glm.model.predictions <- unique(select(d.tj.w.prev.cue, Property, Category, sentence, prevalence, prop))
...
d.tj.w.glm <- left_join(
  d.tj.w.prev.cue %>%
       group_by(Property, Category, sentence) %>%
       multi_boot_standard(column = "response"),
...
ggplot(d.tj.w.glm %>% 
       ...
       aes (x = prediction, y = mean, ymin = ci_lower, ymax = ci_upper, color = cuevalidity))+
  geom_errorbar(alpha = 0.3)+
  geom_abline(intercept = 0, slope = 1, lty = 3)+
  geom_text_repel(data = d.tj.w.glm %>% 
                    rename(cuevalidity = prop) %>% 
                    filter(sqErr > 0.04), 
  ...
  ylim(0,1)+
  coord_fixed()+
  xlab("Prevalence only model prediction")+
  ylab("Human generic endorsement")


rs.glm.2 <- glm(response ~ prevalence + prop, 
              data = d.tj.w.prev.cue, family = 'binomial')

glm.model.predictions.2 <- unique(select(d.tj.w.prev.cue, Property, Category, sentence, prevalence, prop))

glm.model.predictions.2 <- glm.model.predictions %>%
  mutate(prediction = predict(rs.glm.2, ., type = "response"))

d.tj.w.glm.2 <- left_join(
  d.tj.w.prev.cue %>%
       group_by(Property, Category, sentence) %>%
       multi_boot_standard(column = "response"),
  glm.model.predictions.2
) %>% mutate(sqErr = (mean - prediction) ^ 2)
