...
Figuring out that a category tends to have a property, an agent tends to do an action, or a cause tends to produce an effect is crucial knowledge for a cognitive agent.
The productivity of human thought rests upon generalizations.
We communicate generalizations to each other, something which other animals struggle to accomplish.
One important difference between human and non-human animals is the cognitive capacity for language.

A generalization gives us a relation between a category and a property: It tells us to predict that property of that category [@HumeTHN].
...
If one wished to communicate a generalization then, it seems the more direct way would be to convey the probability corresponding to that generalization. 
Ironically, humans are very bad at interpreting probabilities directly [@Tversky1974].
The difficulty we have in understanding numerical descriptions of probability, rather than being a deficit, could reflect the flexible adaptivity of language to rely upon shared context to guide interpretation [@Piantadosi2012].
...
Indeed, generalizations in language are often the simplest kind of compositional expressions (e.g., *Dogs are friendly*, *Mary smokes cigarettes*, *Yeast makes bread rise*). 
...
Rather, these statements exhibit *genericity*; they support generalization to new, unobserved, concrete instances.
...
In this paper, we explore three case studies of generalizations in language: Generalizations about agents, objects, and events. 
...
By doing so, we provide a unified mathematical framework for dealing with agents, objects, and events, casting each as a category over which generalization can occur.
...
In Section 2, we introduce our empirical paradigm through which we will measure endorsement and interpretation of linguisitic generalizations, examining generalizations about *agents* in what is known as *habitual language* (e.g., "Mary smokes cigarettes."). 
...
Suppose we observe instances $\{x_1, x_2, ..., x_n\}$ of category $k$ and some number of them have feature $f$: What do we learn about possible future instances of $x_{n+1}$, particularly with respect to $k$?
...
There are strong limitations to this line of research, however.
...
Though once viewed as a wastebasket in which to dump unexplained loose ends in service of more rigorous formal analysis, *pragmatics* --- or language understanding in context --- has seen a rise in suitable formal frameworks that connect closely with behavioral data [@Franke2015].
...
A simple meaning function would be a threshold on an agent's posterior probability: $\denote{u_{generalization}}(h, \theta): \{ h: h > \theta\}$.
...
Unfortunately, a fixed value of $\theta$ seems unlikely.
...
Finally, *Ducks lay eggs* is a good generalization, though the rate is the same as *are female*.
...
We propose that meaning of a lingusitic generalization is simple but underspecified.
That is, we adopt a threshold semantics as described above, but imagine that $\theta$ is not a fixed property of language.
In this line of thinking, listeners essentially infer thresholds (i.e., meanings) in context.
...
This model is implemented in the probabilisitic programming language WebPPL [@dippl], and a complete, runable version of the model can be found at \url{http://forestdb.org/models/generics.html}
