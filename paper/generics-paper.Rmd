...
glm.gen.endorse.prev <- glm(response ~ prevalence, 
              data = d.gen.endorse.prev.cue, family = 'binomial')

glm.gen.endorse.prev.cv <- glm(response ~ prevalence + prop, 
              data = d.gen.endorse.prev.cue, family = 'binomial')

d.gen.endorse.regression.prevalence <- left_join(
  d.gen.endorse.bayes,
  unique(select(
    d.gen.endorse.prev.cue, Property, Category, sentence, prevalence, prop
  )) %>%
    mutate(prediction = predict(glm.gen.endorse.prev, ., type = "response")) %>%
    mutate(sentence = paste(Category, " ", Property, ".", sep = ""))
) %>% mutate(sqErr = (mean - prediction) ^ 2,
             cuevalidity = prop)
...
d.gen.endorse.regression.prevalence.cuevalidity <- left_join(
  d.gen.endorse.bayes,
   unique(select(
    ...
    mutate(prediction = predict(glm.gen.endorse.prev.cv, ., type = "response"),
...
```{r generic-fullmodel, fig.width = 8}
n_chains <- 1
n_samples <- 100000
burn <- n_samples
...
generics.endorsement.models <- bind_rows(
  ...
  mutate(src = factor(src, levels = c( 
                      ...
                      labels = c(
                                 ...
                                 "Prevalence alone", "Prevalence & Cue validity"
                                 ))) %>%
  ggplot(., aes ( x = prediction, xmin = prediction_lower, xmax = prediction_upper,
                  y = MAP_h, ymin = low, ymax = high, fill = prevalence ))+
  geom_abline(intercept = 0, slope = 1, lty = 3)+
...
```{r generics.endorsement.figure, fig.width=7, fig.height=5}
...
Habituals are a particularly amenable domain to manipulating the target propensity that a speaker aims to communicate.
People are often in the situation of being introduced to new people and learning new information about them.
In addition, the propensity information can be expressed in an easily-interpretable frequency with which a person does an action (e.g., "Last month, Mary smoked cigarettes 3 times.").^[
...
To measure participants' latent prior belief distribution over the frequencies of people doing various actions (e.g., how often people *run*; Expt. 2a), we take advantage of the structured representation discovered in Expt. 1a, and learn about the prior by asking questions about the parameters of this model.
In addition to directly manipulating the target propensity, these experiments provide the first test of the generality of our theory, that it can apply to generalizations about events as well as categories.

Finally, if habituals (and generics) are truly language for conveying generalizations, it would be useful for them to reflect expectations, not merely observations. 
...
In our first investigation of the degree scale, we ask whether $h$ represents the actual, past frequency of the event (e.g., how often Mary has actually smoked in the past week) or the predictive probability that this event will occur in the near future (e.g., how likely it is that Mary will smoke next week)? 
In Expts. 3a \& 3b, we show that $h$, the target of communication, is a speaker’s prediction about the future frequency of action, and not past frequency, tying the phenomenon of *genericity* even more to subjective beliefs.
...
First, we measure the prior distribution over the frequency of events, for different kinds of events.
Then, we test endorsements of habitual sentences, manipulating the actual frequency with which a person does an action.
...
We will assume for these experiments, that those people who have never done the action before will almost never do the action (expected frequency: once every hundred years). 
...
d.hab.priors.tidy <- d.hab.priors %>%
  rowwise() %>%
  mutate(annualRate = annualRates[[as.character(interval)]]*n_times,
         never = ifelse(annualRate == 0, 1, 0),
         action = as.character(action),
         action = ifelse(action == "sell things on eBay", "sells things on eBay", action))

d.hab.priors.distinctiveness <- d.hab.priors.tidy %>%
  group_by(action) %>%
  multi_boot_standard(column = 'never') %>%
  ungroup() %>%
  rename(distinctiveness = mean, 
         distinct_low = ci_lower, distinct_high = ci_upper)
d.hab.priors
d.hab.priors.distinctiveness
# 
# 
# df.out <- df.tidy %>%
...
(1) It smooths effects that are clearly results of the response format. For example, a very common rating for certain events is \emph{1 time per year}. Presumably participants would be just as happy reporting \emph{approximately} 1 time per year (e.g., on average, 1.2 times per year); the raw data does not reflect this due to demands of the dependent measure.
(2) It better captures the tails of the prior distribution (e.g., the probability of a rate of going to the movies two times every five years) which have relatively little data and need to be regularized by the analysis.
...
From the inferred parameters and the assumed functional forms (i.e., the mixture distribution), we can generate $P(h)$ modeled as a mixture of individuals who have done the action before and those that haven't.
...
d.hab.endorsement <- left_join(
  ...
  filter(pass == 1),
  d.hab.priors.distinctiveness %>% rename(habitual = action)) %>%
...
glm.hab.endorse.freq <- glm(response ~ log_times, 
              data = d.hab.endorsement, family = 'binomial')

glm.hab.endorse.freq.distinct <- glm(response ~ log_times + distinctiveness, 
              data = d.hab.endorsement, family = 'binomial')

d.hab.endorse.regression <- left_join(
  d.hab.endorsement.bayes,
  unique(select(
    d.hab.endorsement, habitual, log_times, roundedFreq, distinctiveness
  )) %>%
    mutate(regression_freq = predict(glm.hab.endorse.freq, ., type = "response"), 
           regression_freq_distinct = predict(glm.hab.endorse.freq.distinct, ., type = "response"))
) %>%
  gather(src, prediction, regression_freq, regression_freq_distinct) %>%
  mutate(sqErr = (MAP_h-prediction)^2)

ggplot(d.hab.endorse.regression,
       aes (x = prediction, y = MAP_h, ymin = low, ymax = high, color = distinctiveness))+
...
lag <- 5
i <- 2
model_prefix <- "results-habituals-S1-ffPrior2-3components-normalPrior-silenceAlt-"

m.samp <- data.frame()
for (i in seq(1, n_chains)){
  mi <- fread(paste(project.path,  "models/habituals/results/", model_prefix, 
                    n_samples, "_burn", burn, "_lag", lag, "_chain", i, ".csv", sep = ""))
  m.samp.i <- mi %>% mutate(chain = i) %>% rename(habitual = item, time_period = param)
  m.samp <- bind_rows(m.samp, m.samp.i)
  #print(i)
}

m.hab.somemodel.endorsement <- fread(
  paste(project.path,"models/habituals/results/", 
        "results-habituals-fixedThreshold-S1-ffPrior2HyperStructured-silenceAlt-10000_burn5000_lag5_chain1.csv",
        sep = "")
  ) %>% mutate(chain = 1) %>% rename(habitual = item, time_period = param) %>%
  filter(type == 'predictive') %>%
  group_by(type, habitual, time_period, cat, gender) %>%
  summarize(MAP = estimate_mode(val),
            cred_upper = hdi_upper(val),
            cred_lower = hdi_lower(val))

m.hab.fullmodel.endorsement <- m.samp %>%
  filter(type == 'predictive') %>%
  group_by(type, habitual, time_period, cat, gender) %>%
  summarize(MAP = estimate_mode(val),
            cred_upper = hdi_upper(val),
...
habituals.endorsement.models <- bind_rows(
  ...
  left_join(
    ...
    rename(prediction = MAP, prediction_lower = cred_lower, prediction_upper = cred_upper),
    d.hab.endorse.regression %>% 
    select(habitual, time_period, log_times, distinctiveness)
  )
) %>%
  mutate(src = factor(src, levels = c( 
                                      "generics_model",
  ...
  filter(src == "Speaker model - uncertain semantics") %>%
  ggplot(., aes ( x = prediction, xmin = prediction_lower, xmax = prediction_upper,
                  y = MAP_h, ymin = low, ymax = high, fill = log_times ))+
  geom_abline(intercept = 0, slope = 1, lty = 3)+
  geom_linerange(alpha = 0.7)+
  geom_errorbarh(alpha = 0.7)+
  geom_point(shape = 21)+
...
with(m.hab.endorse.rsa %>% 
  filter(src == "generics_model"), cor(MAP_h, MAP, use='pairwise.complete.obs'))^2

habituals.endorsement.models
```
```{r fig.height = 8}

m.samp %>% filter(type == "param") %>%
  ggplot(., aes( x = val))+geom_histogram()+
  facet_wrap(~habitual)


m.samp %>% filter(type == "prior") %>%
  ggplot(., aes( x = val, fill = time_period))+geom_histogram() + facet_grid(habitual~cat, scales = 'free')+
  theme(strip.text.y = element_text(angle = 0))
...
It is also the first attempt in psychology to try to unify significant swaths of language that are seemingly quite different from one another: generalizations about categories, events, and causes.
...
This scale is defined by a subjective, predictive probability, not mere frequency (Expt. 2).
...
We believe this has been a stumbling block for formal semantic models that try to argue in one or another way for a statistical or probabilty based approach [@Pelletier; @Cohen1999; @Nickel].
As @Nickel2013 notes, genericity is particularly difficult to study because it intersects with many other phenomena in language (e.g., gradable adjectives in *Giraffes are tall*).
...
Our separation of world knowledge from the semantics of generics allows a way for conceptual structure to influence generic production and comprehension, without having to posit conceptual structure and semantics are one in the same.
...
In Expt. 4 we showed that these prevalence distributions are causally related to endorsement.
...
To take a thought-experiment from the linguistics litearature, imagine that currently every Supreme Court justice's social security number is an even number. 
...
Our explanation is this phenomenon is that abstract intuitive theories lead us to reject observed frequencies in forming our subjective probabilities.
...
If the comparison class can be derived with respect to the property [c.f., @Keil1979], one would expect the entities in the class to 

(2) is often used to argue that no actual instances need have occurred to make a generalization true. 
In this thought experiment, part (or perhaps all) of Mary's job is to handle the mail from Antarctica; to date, however, there has not been any mail from Antarctica [@Cohen1999]. 
Thus, it is not the case that Mary has handled the mail from Antarctica in the past.
Our framework takes the fundamental currency of a generalizatoin in language to be a subjective predictive probability. 
In Expt. 3, we manipulate predictive probability and saw the endorsements for habitual language were affected. 
This example bears similarity to the *enabling* condition of that experiment, wherein causal events increased the propensity of the event in the future, and also resulted in an increase in endorsements of habitual language.

(3) shows a subtle interaction of generic language with another linguistic device: conjunction. 
(3) sounds true but if one takes the conjunction to apply to the properties, it is not true.
That is, we don't think there to be international elephants (i.e., individual elephants who live in Africa part time and Asia part time).
...
With reasonable assumptions about the prior (e.g., assuming international elephants are unlikely), upon the hearing "Elephants live in Africa", the moddel infers that most elephants live in Africa.
...
This is a nice feature of the uncertain threshold semantics: flexibility in accomodating new evidence that might otherwise contradict previous statements.

Finally, (4) is used to argue that even if the property is widespread (perhaps in a soft sense, as in our model, relative to other kinds or groups), we might find it avervise to actually assert the generic.
That is, even if it were the case that, because of a variety socio-cultural reasons, women tend to be submissive, we might still not want to say *Women are submissive* [@Haslanger]. 
...
If the generic conveys the property is in fact more widespread than the speaker herself might believe, this can lead to a distortion of the truth [cf., @Cimpian2010].
The current formulation of the model is relatively ambivalent the deviations of the listener's beliefs from the speaker's beliefa.
Other models in the RSA framework have identified states of the world with different subjective values (e.g., how good of a job a listener did on a task), and speakers with utilities that extend beyond conveying information can produce qualitatively different patterns of utterances [e.g., polite speech; @Yoon2016; @Yoon2017].
If our model believed that states of the world where the prevalence of the feature is high (e.g., a world where many women were submissive) were undesirable, then a speaker who considered both the informational content of the utterance and the utility in the states of the world implied by that utterance would too find it reprehensible to make such generic claims. 
