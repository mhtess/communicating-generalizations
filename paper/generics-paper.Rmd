...
To understand the contribution of our model, we articulate plausible alternative models that have been proposed in the literature. 
...
If prevalence is the "forward probability" (e.g., one's predictions about whether an entity will lay eggs after learning that the entity is a robin), *cue validity* is the inverse probability: $P(k \mid f)$ (e.g., one's predictions about whether or not the entity is a robin, upon learning that it lays eggs).
...
For example, "Robins lay eggs" still receives only intermediate endorsement by this model, and this highlights a shortcoming of the summary statistic of cue validity. 
...
Negative properties (like, "not carrying malaria") are completely undiagnostic for almost every category, and this model ends up believing "Mosquitos don't carry malaria" is a relatively good statement because of the high prevalence (i.e., most mosquitos don't carry malaria).
...
Rather than fitting the parameter, we inferred it and integrated over their plausible values using Bayesian data analysis [@LW2014].
