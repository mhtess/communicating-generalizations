...
Figuring out that a category tends to have a property, an entity tends to exhibit a behavior, or a cause tends to produce an effect is crucial  knowledge in an uncertain world. <!-- can be crucial and hard-won knowledge. -->
Without generalizations, human thought would be confined to direct experience: We would be unable to predict if the lion was about to charge or if eating a certain plant could be deadly.
Generalizations are the life-blood of cognition and they are the foundations of generative, productive thought.
From a very young age, children generalize from their direct experience to new situations or novel instances of a category [@Dewar2010; @Gelman1986]. 
(CHECK THESE CITATIONS).
The human capacity to generalize from experience displays subtle sensitivities to context [@Nisbett1983].
Quantitative modeling has been crucial in informing our theories of how observers form generalizations.
Inductive inferences are not subject to not only the details of the observations, but also how those observations are generated [@Gweon2010; @Shafto2012].
...
It is not surprising then that language provides simple ways to communicate generalizations: *Dogs are friendly*, *Mary smokes cigarettes*, *Yeast makes bread rise*. 
...
In recent years, there has been tremendous progress in quantitative models of language use that display complex sensitivites to context [@Goodman2016; @Franke2015].
The Rational Speech Act theory of language use is one such family of models that has succeeded, where others have failed, to explain *pragmatic* language phenomena (i.e., interactions between language and context).
This framework has been used to explain, in precise mathematical terms, how it is that utterances like "some of the kids failed the test" are interpreted to mean "some (but not all) of the kids failed the test" [@Goodman2013], how "I waited a million years to get a table" is interpreted as "I waited a long time and I was annoyed" [@Kao2014], and how "John is tall" can mean John is 5'4" is John if a 12-year-old but will mean John is 6'2" is he is 24-years-old [@Lassiter2015].
...
A formal theory of *general* statements about entities or situations that extend beyond the here-and-now has remained elusive.
...
At first it seems the generalizations conveys something about the propensity or probability of an instance, of a property, or an event to occur, but exactly what is conveyed is unclear.
*Mary smokes cigarettes* seems to suggest a daily-habit, while *Bill writes poems* implies something much weaker, maybe a poem every few weeks.
...
Generalizations, it seems, do not reduce to precise quantitative statements; this is the major hurdle in providing a formal account of generalizations in language [@Carlson1977; @Leslie2008; @NickelBlackwell].

One particular kind of linguistic generalization has been studied extensively in cognitive and developmental psychology: *generic* statements, or generalizations about categories.
...
This characteristically strong interpretation (phenomenon 1) interacts with the flexibile truth conditions to exaggerate evidence (phenomenon 2). 
...
We take as a starting point the idea that the core meaning of a linguistic generalization is simple (it conveys something about the propensity), but underspecified.
We situate this idea inside a rational model of communication, and find that general principles of pragmatic reasoning can be used to resolve more precise meanings in context. 
We apply this framework to explain generalizations about categories, events, and causal forces. 
