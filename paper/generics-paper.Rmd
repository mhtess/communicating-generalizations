...
Figuring out that a category tends to have a property, an agent tends to do certain actions, or a cause tends to produce an effect is crucial knowledge in an uncertain world.
The productivity of human thought rests upon generalizations.
Human beings convey generalizations to each other, a capacity that seems to be lacking in non-human animals. 
One key difference is that language provides simple and ubiquitous ways to communicate generlizations. 

A generalization gives us a relation between an object and a property: It tells us to predict that property of that object [@HumeTHN].
Generalization can then be described as a probability and inductive inference as a probabilistic model [@Tenenbaum2011].
...
To communicate generalizations, then, it seems the simplest thing would be to convey the probability.
Ironically, humans are very bad at interpreting probabilities directly [@Tversky1974].
Rather than being a deficit, the difficult we have in understanding numerical descriptions of probability could reflect the flexible adaptivity of language to rely upon shared context to guide interpretation [@Piantadosi2012].
If that's true, we would expect the language of generalizations to be simple.

Indeed, generalizations in language are often the simplest form of compositional expressions e.g., *Dogs are friendly*, *Mary smokes cigarettes*, *Yeast makes bread rise*. 
There are numerous syntactic forms all corresponding to generalizations: "*Dogs* are friendly", "*A library book* should be treated with respect", "*The contemporary foodie* seeks culinary experinces in menu-less restaurants and grandmothers' kitchens" [@Carlson1995; @NickelBlackwell; @sepGenerics].
Each of these statements predicates a property of a category, but they are not properties that apply only to categories like *Dinosaurs are extinct*. 
Rather, these statements support generalization to new, unobserved instances.

In this paper, we explore how speakers convey generalizations and how listeners interpret them. 
...
Computational modeling has deepened scientific understanding of complex linguistic phenomena such as politeness [@Yoon2016], hyperbole [@Kao2014], and vagueness [@Lassiter2013]. 
Until now, however, rational models of language use have focused on statements about *specific* situations: utterances that convey information about a particular entity or event. 
A formal theory of statements that convey generalizations about entities or events --- statements that extend beyond the here-and-now --- has been heretofore unexplored.

In this paper, we explore three case studies of generalizations: Generalizations about agents, objects, and events. 
In Section 1, we introduce an information-theoric communicative theory of how generalizations are conveyed in language and flesh out the model's predictions along with some alternative models.
By doing so, we provide a unified mathematical framework for dealing with agents, objects, and events, casting each as an instance of a category.
Our model predicts the truth conditions of generalizations should vary with the (A) the probability of an instances having the property and (B) the prior distribution over the property probability (e.g., how likely instances of other categories are to have the property).
In Section 2, we introduce our empirical paradigm through which we will measure endorsement and interpretation of linguisitic generalizations, examining generalizations about *agents*. 
In Section 3, we extend our paradigm to generalizations about *events* and experimentally manipulate participants' background knowledge, showing that the theoretical constructs we posit are causally related to endorsement and interpretation of generalizations. 
