---
title             : "Communicating generalizations about events, causes, and categories"
...
Even before birth, human and non-human animals like gather statistical information about the world in order to redict what will come next.
Humans in development, perhaps uniquely, show striking sensitivity to intentional and pedagogical actions, drawing stronger inferences than would otherwise be warranted from random observations.
At around the age of two, the human mind gains access to a unique source of data: language.

At around the age of two and a half, children begin to understand that generic language refers to kinds and conveys generalizations that extend beyond the here and now [@Cimpian2008].
Generic language then grows conceptual knowledge [@Gelman2009].

Computational models of learning formalize the inferences derived from observations.
They showcase the role of structured representations [@Kemp] and assumptions learners can make about the data-generating process [@Shafto]. 
Computational models of learning focus on the kind of data rich in the early period of development and still have a central purpose in learning procedural knowledge from pedagogy. 
At some point, linguistic input becomes very salient 

Learning from language accelerates conceptual development, but this kind of learning has eluded formalization.
Probabilistic models have recently been able to capture the subtle sensitivity to context that is required to understand language [@Frank2012].
These models, however, have only dealt with utterances describing concrete situations or particular entities.
The language of generalizations, however, has eluded precise formalization.

The major issue is deciding what makes something true or false. 

Without knowing what makes it true or false, it's also not clear what inferences may be drawn from it.

In this paper, we combine a probabilistic model of pragmatic reasoning with ideas from formal semantics to define the truth conditions of generic language. 
By formalizing such a hypothesis in a probabilistic language understanding model, predictions for endorsement and interpretation of generic language are generated by specifying relevant prior knowledge.
By both measuring and manipulating prior knowledge, we show that it is causally related to generic endorsement and interpretation in the ways predicted by our model.
...
In each domain, we define a scale that corresponds to the relevant metric by which to generalize. 
We show that in each domain, our general theoretical framework applies and predicts endorsements and interpretations about generalizations conveyed in language. 

The rest of this paper is organized as follows. 
In Section 1, we describe the Bayesian framework for generalization and define the metric scale along which generalization is measured. 
...
Our model makes predictions about how interpretations should vary according to (i) the listener's world knowledge prior, and how endorsement should vary according to (i) the world knowledge prior and (ii) the speaker's intended message (or, belief distribution) that she seeks to communicate. 
In the following three sections, we test these predictions in three domains for communicating generalizaions: categories, events, and causes.
In Section 3, we describe experiments the measure both (i) and (ii) and relate it to the endorsement and interpretation of generalizations about categories, or *generic language*. 
In Section 4, we describe experiments that measure (i) and manipulate (ii) and relate it to endorsement and interpretation of generalizations about events, or *habitual language*.
In Section 5, we describe experiments that manipulate both (i) and (ii) and predict endorsement and interpretation of generalizations about causes, or *causal language*. 
Across all of our experimental settings, we find a very strong agreement of our model's predictions to both the endorsement and interpretation behavioral data (cumulative $r^2(N) = 0.XX$).
...
<!-- Humans communicate generalizations to each other, -->
...
Until now, however, rational models of communication have focused on statements about *particular* situations: utterances that convey information about a specific entity or event. 
...
We can say "My dog is friendly.", "John ran this afternoon.", or "My friend drank moonshine and it made him go blind."
...
We will discuss communicating generalizations in the context of generalizations about categories (i.e., *generic language*), though the arguments will extend to events and causes as well. 
...
Other generics convey properties that are *characteristic* of a kind, such as *Robins lay eggs*, argued to have a *principle connection* between kind and property [@Prasada2006; @Prasada2013].
...
Is there some more general, unified principle that explains generic truth conditions?

Bayesian cognitive models are used to represent rich, structured knowledge of the world and formalize the inferences that people draw using such conceptual representations [e.g., @Kemp, @Goodman2015Chapter].
The basic currency of inference in these models is probability.
In the case of generic language, conceptual factors may influence subjective probability, which in turn influence the meaning of a generic statement.
...
Figure \ref{fig:simulations} shows model $L$'s posterior distribution on $h$ upon hearing utterances corresponding to fixed thresholds at 0 and 0.5 ("some" and "most"), as well as the uncertain threshold (Eq. \ref{eq:L0}).
...
"Most" rules out all $h$ below 0.5 and is agnostic among the $h > 0.5$.
...
When the listener aggregates over these thresholds, higher values of $h$ pass more thresholds; thus, higher $h$ are more likely *a posteriori*.

In this paper, we are interested in endorsements of generalizations based on some known or believed probability $h$.
We describe an endorsement model as a Bayesian decision of whether or not to say the generalization to the listener $L$ (Eq. \ref{eq:L0}).
\begin{equation} 
S(u \mid h) \propto \exp{(\lambda \cdot \ln{ \int_{\theta} L_0(h, \theta \mid u)}  \diff \theta  }
\label{eq:S1}
\end{equation}

This is a Rational Speech Act (RSA) model, where language production and comprehension are defined by recursive Bayesian models describing speaker and listener [@Goodman2016].
Following RSA, this speaker is an approximately rational Bayesian agent with degree with rationality governed by parameter $\lambda$.
...
priorShapes <- list(
  ...
  hasWings =  list(params = data.frame(a = 50, b = 1), mix = 0.4)#, 
  #areSick =  list(params = data.frame(a = 1, b = 5), mix = 1), 
  #carriesMalaria =  list(params = data.frame(a = 1, b = 5), mix = 0.4)
)
...
all.simulations %>%
  mutate(Distribution = factor(Distribution, levels = c("Prior", "Posterior_0", "Posterior_05", "Posterior_Generalization"),
                               labels = c("Prior/Silence", "Some", "Most", "Generalization")),
         PriorShape = fct_relevel(PriorShape, "uniform", "isFemale", 
  ...
  ggplot(., aes(x = value, #y = prob, 
    ...
    scale_fill_manual(values = c("#636363", "#d7191c"))+
...
Generalizations about categories (i.e., *generic language*) have received substantial interest of psychologists for their role in concept formation [@Gelman2004] and stereotype propogation [@Rhodes2012].
Generic language (e.g., *Birds fly.*) exhibits similar flexibility in meaning and truth conditions as other kinds of generalizations in language.
...
As a first test of our theory, we measure the relevant prior knowledge for the Bayesian model and show how it systematically predicts endorsements of familiar generic statements (Expt. 1) and interpretations of novel generic statements (Expt. 2). 
...
We then compare the model's predictions for endorsement to empirically measured endorsements of generic sentences (Expt. 1b).
...
Then ask yourself, "what percentage of that kind of animal is female?".
...
This decomposition of the prevalence prior $P(h)$ into a kind prior $P(k)$ and then a conditional probability of the prevalence of the feature given the kind $P(h \mid k)$ is one way to measure to the prevalence prior $P(h) = \int_{k} P(h \mid k) \diff k$.
...
We begin by exploring the hypothesis prior is structured as a result of deeper conceptual knowledge.
This is one way in which our theory can touch upon other psychological accounts of generic language [cf., @Prasada; @Gelman; @Leslie].
If, indeed, there were such things as different types of kind--property relations (e.g., *principled* v. *accidental relations*), we would expect the prior data to follow a multi-modal, mixture distribution where different modes or components of the prior are the results of the different types of relations.
...
Because the Beta distribution is not defined at the points 0 and 1, we add $\epsilon$ to the 0 responses and subtract $\epsilon$ from the 1 responses.
...
We model this distribution is a Beta distribuition that heavily favors values near 0: $\text{Beta}(1, 100)$.
The "stable" distribution is modeled as a Beta distribution with unknown parameters $\text{Beta}(\alpha, \beta)$.
In addition, we assume that these two components combine with some weighting $\phi$ such that the data we observe is $$P(d) = \phi\cdot \text{Beta} (d \mid \alpha, \beta) + (1 -  \phi) \cdot \text{Beta}(d \mid \alpha = 1, \beta = 100) $$.
...
We construct a posterior predictive distribution by first flipping a coin weighted by $\theta$.
...
We do at each run of the model to generate a distribution over predicted prevalence ratings.


We implemented this statistical model using the probabilistic programming language WebPPL [@dippl].
To learn about the credible values of the parameters, we ran separate MCMC chains for each item, collecting 50,000 samples and discarding an additional 25,000 for burn-in.
...
d.gen.endorse.catch <- read.csv(paste(project.path, data.path, 
                    "truth-judgments_catch-trials.csv", sep = ""))

d.gen.endorse <- read.csv(paste(project.path, data.path, 
...
To understand the contribution of our model, we articulate plausible alternative models that have been proposed informally in the literature. 
We describe these two alternatives before describing the fit of our model.

From the prevalence prior data (Expt. 1a), we estimate participants' beliefs about the prevalence of a property \emph{for particular kinds} (e.g., the percentage of \textsc{robins} that \textsc{lay eggs}; see green intervals on Figure \ref{fig:commongenerics}a insets, and Table 2 in Appendix).
...
This is not surprising given that our stimulus set includes generics that are true with high-prevalence properties (e.g., \emph{Leopards have spots.}) and  generics that are false with low prevalence properties (e.g., \emph{Leopards have wings.}). 
...
*Cue validity* is the inverse probability of prevalence: $P(k \mid f)$; it represents the salience of a category given a feature but also captures the distinctiveness of the feature. 
...
Previous studies have found cue validity to be correlated with generic endorsement [@Khemlani], though cue validity has been argued against on *a priori* grounds [@Leslie].

For a prevalence prior with a fixed set of categories, cue validity can be calculated from the prevalence prior distribution (i.e., it is a summary statistic of this distribution).
In empirical studies of generic language, however, it is often empirically measured.
To measure cue validity, we employed a free production paradigm, following @McRae.
...
A linear model that uses predictors for both prevalence and cue validity does a better job at explaning the endorsement data ($r^2(30) = 0.79$).
...
For example, "Robins lay eggs" still only receives only intermediate endorsement by this model, and this highlights a shortcoming of the summary statistic of cue validity. 
...
Cue validity is too blunt a metric to capture this subtlety.
The enriched alternative model also falls short for "Mosquitos don't carry malaria".
Negative properties (like, "not carrying malaria") is completely undiagnostic for almost every category, and this model ends up believing "Mosquitos don't carry malaria" is a relatively good statement because of the high prevalence (i.e., most mosquitos don't arry malaria).
...
We use the empirically estimated within-kind prevalence as the $h$ that the speaker $S_1$ is trying to communicate, and use the empirically measured priors from Expt. 1a as the listener's prevalence prior $P(h)$.^[
...
Rather than fitting the parameter, we inferred them and integrated over their plausible values using Bayesian data analysis [@LW2014].
We put an uninformative prior over this parameter, with a range consistent with previous literature using the same model class: $\lambda_1 \sim \text{Uniform}(0,5)$.
...
By measuring listeners' prior beliefs about the prevalence of the property across kinds and the prevalence of the target kind (e.g., the percentage of birds that lay eggs), we were able to show how an underspecified-threshold semantics is tenable inspite of these seeming counterexamples.
...
In explaining these three phenomena (variable endorsements, implied prevalence, and the asymmetry), we measured the prior distribution over the prevalence of the features across categories. 
In addition, for the endorsement data, we had to measure the prevalence of the feature in the target category (e.g., the percentage of robins that lay eggs) to model the endorsement data.
Thus, this evidence is largely correlational in nature. 
...
In our second case study, we continue to measure the prior distribution but manipulate the target probability.
We do this in the domain of generalizations about events (i.e., *habitual language*).
In this case study, we focus on a particular class of events: people doing actions.
While we could manipulate the target probability for generic endorsements (e.g., the prevalence of feature in a kind), this may interact with domain knowledge in complex ways (e.g., it's *a priori* very unlikely that 90% of a hypothetical kind---lorches--- have broken wings).
Instead, we carry out this manipulation in the case of events, where manipulating target probability is quite straightforward.
In this domain, target probabilities correspond to the frequency with which a person does an action.
In addition to generalizing our theory to a novel domain, these experiments provide the first test of the generality of our theory.

*Habitual* language (e.g,. "Mary smokes."; "My car won't start.") convey generalizations about events. 
As a first test of our communicative theory, we examine the conditions under which such statements are endorsed as well as the frequencies implied by such statements. 

This set of experiments is organized as follows.
In the **Event Prior Elicitation** study (Expt. 1), we measure participants' latent prior belief distribution over the frequencies of people doing various actions (e.g., how often people *run*).
...
### Experiment 4a: Measuring the propensity prior for event knowledge
...
## Experiment 4b: Endorsing generalizations about events
...
We next explore the endorsements of habituals of this form made from the items whose priors were measured the prior elicitation task. 
...
A simple null hypothesis is that habitual statements convey that the a person *has done this action before*. 
...
## Experiment 5: Communicating predictive propensities

In Experiment 4b, we supplied participants with a statement about how often a person has done the action in the past and asked them to judge the correspoding habitual statement.
...
In Expt. 4a, we measure *predictive frequency* both when past frequency alone is observed and when these causal factors are introduced.
In Expt. 4b, we examine felicity judgments of the habitual sentence (e.g., \emph{John smokes cigarettes.}; \emph{Susan eats peanut butter.}) in the presence of these causal modifiers.
...
### Experiment 5a: Prediction elicitation
...
On all trials, participants were presented with a \emph{past frequency sentence} (same as Expt. 2).
...
### Experiment 5b: Felicity judgments
...
None of the participants had completed Expt. 4a.
%\subsubsection{Procedure and materials}
The only difference from Expt. 4a is the dependent measure. 
...
We use the mean predicted log frequency from Expt. 4a as the input to the speaker $S_2$ model to predict the felicity judgments measured in Expt. 4b.
...
So far, we have seen that our computational model endorses and interprets statements that communicate generalizations about events in a way comprable to how humans both endorse and interpret such language.
The model decides if a habitual sentence is a pragmatically useful way to describe the rate at which a person does an action, taking into account the listener’s prior beliefs about the action--—how common it is and the likely frequency (measured in Expt. 1). 
In Experiments 2, we varied the type of event and the past frequency with which the person did the action, and found substantial graded variability in the endorsement judgments of people.
...
In Experiment 4, we further investigated the nature of the underlying “propensity” scale by introducing enabling and disabling causal evidence, measuring the predicted future frequency (Expt. 4a) and using that, with the model, to predict the felicity of habitual sentences (Expt. 4b). 
...
In the remaining sections of this paper, we extend our theory to communicating generalizations about causes and categories, and provide further tests for our theory in these domains.
...
We design this set of experiments with the goal of testing whether:
(1) background knowledge that gives rise to the prevalence priors is causally related to the interpretation and endorsement of causal statements, and 
(2) abstract domain theories influence interpretation and endorsement of causals via the prevalence prior.
Also, by designing the experiments in the domain of causal language, we also demonstrate the generality of our theory of communicating generalizations.


In Experiment 5, we manipulate participants' beliefs about a causal system and measure their updated beliefs about causal power.
In Experiment 6, we test whether these manipulated priors give rise to different endorsements of causal statements.
In Experiment 7, we test the influence of these manipulated priors on interpretations of causal language.
We conclude this section by comparing RSA models to the endorsement and interpretation data.


## Experiment 6a: Manipulating the Prevalence Prior for Causes
...
## Experiment 6b: Endorsing Causal Language

In this experiment, we tested whether the manipulated priors of Expt. 5 are causally related to the endorsement of casusal statements.

### Method

Most of the experiment was identical to that of Experiment 5.

#### Participants

We recruited N participants from Amazon's Mechanical Turk.
Participants were restricted to those with U.S. IP addresses and who had at least a 95\% work approval rating.
None of the participants had participated in Experiment 5. 
The experiment took on average N minutes and participants were compensated \$N.NN for their work.

#### Procedure and materials

The materials were the same as in Experiment 5.
Phase 1 of the experiment was identical to that of Experiment 5.
In phase 2 of the endorsement task, the table and story were removed from the screen and the participant is told that the results of the "lost experiment" were found. 
