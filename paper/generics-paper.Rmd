---
title: "Words are vague: Communicating generalizations about categories"
short-title: "Communicating generalizations about categories"
...
author-note:
    "Correspondence concerning this article should be addressed to: M. H. Tessler, Stanford University, Department of Psychology, 450 Serra Mall, Bldg. 420, Rm. 316, Stanford, CA 94305, mtessler@stanford.edu, 757-561-7971."
    
abstract: 
    "Acquiring abstract knowledge requires making generalizations.
    Everyday generalizations typically rest upon not observational data though, but upon testimony: Generalizations conveyed with language.
    The language of generalizations---so called, generic language (e.g., *Dogs bark.*)---appears early in development and is ubiqutious in everyday conversation.
    However, what makes generic sentences true or not depends upon the context, generics are interpreted quite strongly, and they exaggerate evidence. 
    These three phenonena have confounded previous attempts to formalize the meaning of generalizations in language.
    ...
    Our model shows these three central phenomena emerge from the interaction of diverse prior beliefs with communicative principles.
...
Figuring out that a category tends to have a property, an entity tends to exhibit a behavior, or a cause tends to produce an effect can be crucial and hard-won knowledge. 
It is not surprising then that language provides simple ways to communicate such generalizations: *Dogs are friendly*, *Bill smokes cigarettes*, *Yeast makes bread rise*. 
Generalizations are among the easiest forms of compositional linguistic expression and appear everywhere in human conversation (*Movie theaters are cold.*), pedagogy (*Blickets make the machine go.*), political and scientific discourse (*The President peddles falsehoods.*, *Psychology experiments don’t replicate.*), stereotypes (*Boys are good at math*), motivation (*Big girls eat their broccoli!*), emotion-regulation  (*Accidents happen.*), child-directed (*Mommy works late*) and child-produced speech (*Slides aren't for grown-ups!*), and many other facets of experience [@GelmanEtAl2004; @Gelman2008; @Cimpian2010motivation; @Rhodes2012].
...
Intuition suggests that generalizations in language convey something about the propensity of an instance, property, or event, but exactly what is unclear.
*Bill smokes cigarettes* seems to suggest a daily-habit, while *Susan writes novels* implies something much weaker, maybe a novel every few years.
...
*Mosquitos carry malaria* is a true statement despite the prevalence of carrying malaria among mosquitos being a just a fraction of 1\%.
The fact that these generalizations do not reduce to particular quantitative statements implies that what makes them true or false (in terms of precise quantitative measurements) is also underspecified [@Carlson1977; @Leslie2008].

Yet, listeners still have intuitions about how strongly a generalization is expected to hold.
The case of *Mosquitos carry malaria* suggests the generic must in some way be analogous to "some" (i.e., *Some mosquitos carry malaria*). 
Yet, generalizations about categories are often interpreted as implying the property is widespread: *Swans have hollow bones* means something  different than *Some swans have hollow bones* [@Gelman2002].
How particular interpretations arise from generalizations in language is also a mystery.

Finally, generalizations conveyed in language can exaggerate evidence.
This phenomenon emerges when considering the first two (flexible truth and strong interpretations) in the same situation.
...
<!-- Our theory formalizes the idea that the core meaning of a linguistic generalization is simple, but underspecified, and that general principles of communication may be used to resolve the precise meaning in context.  -->
...
In Section 1, we introduce a unified formalism for generalizations and a information-theoric communicative theory of generalizations in language.
...
Here, we restrict our focus to generlizations about categories (generic langauge), generalizations about events (habitual language), and generalizations about causal forces (causal language). 
Generalizations are used to make predictions about instances that an agent has yet to experience [@HumeTHN].
Thus, in order to define a generalization about a category, event, or causal force, there must be some corresponding concrete particular instance of that category, event, or causal force from which the generalization is formed. 
