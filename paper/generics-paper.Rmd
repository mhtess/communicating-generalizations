...
Social learning---relying upon the human capacity to infer others' intentions---faciliates this process [@Gergeley2005; @Tomasello; @Butler2012].
...
It's plausible that cultural knowledge accumulates across generations through language's ability to talk about generalizations [@Tessler2017; @Henrich2015].
...
Most strikingly, the threshold variable may be better modeled as pragmatically "lifted variable", where it's resolution in context depends upon both what would make the utterance as well as informative [a so-called uRSA model; @Goodman2016].
...
For purposes of illustration, we present present predictions of the simplest model where the threshold is not uncertain but is fixed to either 0 or 0.5 (corresponding intuitively statements like *some of the time* or *most of the time*).
...
d.hab.endorsement.summary <- df.endorse %>% 
  mutate(response = ifelse(response == "agree-key", 1, 0),
