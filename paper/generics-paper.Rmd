...
Minimally, language provides labels, which help foster generalization [@Gelman; @Markman].
Language itself can also convey generalizations.
...
Learning from language, however, is more challenging to formalize because of the inherent context-sensitivity of language [@Grice; @Clark1996; @Levinson2000].
Recently, probabilsitic models have made progress in describing pragmatic inferences in precise terms [@Frank2012; @Goodman2013; @Goodman2016], describing complex linguistic phenomena such as politeness [@Yoon2016], hyperbole [@Kao2014], and vagueness [@Lassiter2013].
...
The language of generalizations --- statements that extend beyond the here-and-now, however, has heretofore eluded precise formalization.
...
It doesn't have to do with the fact that the property only applies to females, though, because *Robins lay eggs* is intuitivley true.

Looking beyond generic statements, there is a large swath of language that seem to convey generalizations but about different things.
*Genericity* can be observed when talking about categories (e.g., *Dogs bark*), people in general (e.g., *You never know what will happen on a blind date*), specific people (e.g., *Martha swims after work*), and causal events (e.g., *Drinking moonshines makes you go blind*).
...
d.gen.endorse.priors <- read.csv(paste(project.path, data.path, "naturalGenerics-prior-trials-n57.csv", sep = ""))
...
var model = function(){
  var phi = uniformDrift({a:0, b: 1, width:0.05})
...
In Experiment 4a, we manipulate participants' beliefs about a causal system and measure their updated beliefs about causal power.
