...
m.gen.somemodel.endorsement <- fread(
  paste(project.path,"models/generics/results/", 
        "results-fullModel-s1-smtncssome-20000_burn10000_chain1.csv", sep = "")
  ) %>% mutate(chain = 1) %>%
...
d.hab.endorse.regression.freq.distinct <- left_join(
  d.hab.endorsement.summary,
  unique(select(
...
ggplot(d.hab.endorse.regression.freq.distinct,
...
with(d.hab.endorse.regression.freq.distinct %>%
