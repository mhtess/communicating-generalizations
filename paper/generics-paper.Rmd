...
Figuring out that a category tends to have a property, an entity tends to exhibit a behavior, or a cause tends to produce an effect is extraordinarily useful knowledge in an uncertain world. <!-- can be crucial and hard-won knowledge. -->
It is not surprising then that language provides simple ways to communicate such generalizations: *Dogs are friendly*, *Mary smokes cigarettes*, *Yeast makes bread rise*. 
...
At first it seems the generalizations conveys something about the propensity of an instance, property, or event, but exactly what is conveyed is unclear.
...
*Mosquitos carry malaria* is a true statement despite the prevalence of carrying malaria among mosquitos being a just a fraction of 1\%, while *Canadians are right-handed* might seem like a weird thing to say, even though the vast majority of Canadians are.
Generalizations, then, do not reduce to particular quantitative statements; this is the major hurdle in providing a formal account of generalizations in language [@Carlson1977; @Leslie2008; @NickelBlackwell].

Generalizations about categories (in the form of *generic language*) have been studied extensively in cognitive and developmental psychology.
...
In this experiment we elicit the prior $P(p)$ for different events in order to generate model predictions for corresponding habituals.
...
Participants answered both questions for each gender on each slide (4 questions total per slide, order of male / female randomized between-subjects), and every participant completed all 31 items in random order.
