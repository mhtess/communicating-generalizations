...
abstract: > 
    Generalizations are the foundation of abstract thought and are naturally formalized in Bayesian models of cognition. 
    The *language* of generalizations (e.g., *Birds fly.*), however, has received comparatively little attention by formal psychological models, despite its ubiquity in everyday discourse and child-directed speech.
    ...
    To test this theory, we examine 3 case studies: generalizations about events (i.e., *habitual language*, e.g., *John runs*), causes (i.e., *causal language*, e.g., *The block makes the machine play music.*), and categories (i.e., *generic language*, e.g., *Birds fly*), and find that the model, and not simpler models, explains both the endorsement and interpretation of generalizations in language.
...
Crucially, hard-won generalizations can passed among agents. 
...
Each of these statements has in common the fact that they predicate a property of a category and the properties themselves apply to instances of the category (unlike, e.g., *Dinosaurs are extinct*). 
...
To understand how generalizations are conveyed in language, we start by considering general purposes language understanding principles.
...
## Experiment 1: Event Prior Elicitation
...
Leraning that one event consistently tends to result in another event serves as the foundation for building an intuitive theory of causailty.
...
It's useful to divide labor and have some members research which modifications to the food preparation procedure are good modifications while others look into different ways of cutting stone. 
...
In this domain, an instance of the category is a single causal event (e.g., adding a particular amount of a particular kind of yeast to an actual at a particular time of the day). 
@Gerstenberg
...
We design this set of experiments with the goal of showing that:
(1) background knowledge that gives rise to the prevalence priors are causally related to the interpretation and endorsement of causals
...
## Experiment 5: Endorsing and Interpreting Causal Language

## Method

The experiment is a single-trial experiment with 2 phases to the trial.
In the first phase, participants are given a story about an experimental domain in question.
This information is common to all 3 between-subjects conditions.
...
Participants re that they are an astronaut-scientist on a distant planet trying to figure out how some system works (e.g., how to make a certain kind of animal more alert). They are told:
...
Eleven "experiments" are shown, though one of the experiments had their results misplaced so they cannot report on the results at that point.
...
Upon proceeding to phase 2, the results of the "experiments" are removed and participants are assigned to one of three conditions: (a) prior / manipulation check; (b) endorsement; or (c) interpretation. 
We will describe each of those, and then discuss the materials in more detail.

### Conditions

#### Prior / manipulation check

In the first part of the trial (described above), participants are shown a distribution of results of experiments.
We do not assume that the distributions shown in the trial are directly encoded as participants' prior beliefs about causal power.
People have higher-order beliefs about diferrent kinds of causal systems.
To highlight just one dimension of variability, some systems are likely to have deterministic cause-effect relations (e.g., physical systems), while others be more permissible of probabilistic causes (e.g., behavioral, psychological, or social systems). 
We harness this variability in the design of our materials (described below), and measure participants' nwe beliefs about the domain in question, after having seen the results of the experiment.
Thus, we manipulate participants' prior beliefs and then measure their resulting predictions about future instances of domain. 
We expect the manipulation by way of "experiments" to interact with participants' domain knowledge.

When the participant clicks to advance to the next slide (phase 2), the results of the experiments are removed and participants are told they there were some more experiments conducted, and they were asked to predict the results of the next 4 experiments.
...
In phase 2 of the endorsement task, the participant is told that the results of the "lost experiment" were found. 
...
In phase 2 of the endorsement task, the participant is told that the results of the "lost experiment" were found and that their colleague has reported to them about the results using the causal: "Treatment X makes the effect happen". 
...
## Experiment 5: Endorsing Generic Language


### Measuring prevalence and the prevalence prior
...
## Experiment 6: Interpreting Generic Language (Replication and Extension of Cimpian et al, 2010)

## Experiment 7: The asymmetry between interpretation and endorsement (Replication and Extension of Cimpian et al., 2010)
