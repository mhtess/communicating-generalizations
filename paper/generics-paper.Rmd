...
Inferences drawn inductively from observational data is but one component of the human capacity for learning generalizable knowledge.
When properties are difficult to observe (e.g., *cows have four stomachs*) or events statistically unlikely (e.g., *lightning strikes tall objects*), it is important to be able to learn this information through means other than direct observation.
...
Despite the ubiquity of generalizations in language, the human capacity to generalize from observations has been studied far more extensively in cognitive and developmental psychology [@Nisbett1983; @Heit2000].
...
There is considerable linguistic evidence that a computational understanding of communicating generalizations is nontrivial.
...
Suppose we observe instances $\{x_1, x_2, ..., x_n\}$ of class $k$ and some number of them have feature $f$: What do we learn about possible future instances of $x_{n+1}$, particularly with respect to $k$?
For features among entities of a category or an effect following a cause, the relevant propensity $h$ is a probability $h = P(x \in f \mid x \in k)$^[
This can also be called the *prevalence* of $f$ among $k$.
].
Here, $x$ is an object.
For a behavior exhibited by a entity, the relevant propensity is a rate --- how frequently (in time) does $x \in f$ occur --- which can be described as a probability integral: $h = \int_{t} P(x \in f_t \mid x \in k_t) \diff t$
Here, $x$ is a instance.
We can treat propensity as an unknown latent cause of $f$ in $k$, and use Bayesian inference to learn from our observations.

<!-- Suppose *a priori* we have no informative beliefs about the propensity (i.e., $P(x \in f \mid x \in k) \sim \text{Uniform}(0, 1)$). -->
Observing some number of positive instances of $k$ with $f$ will update a prior belief distribution (perhaps in an uninformed prior) into a posterior belief distribution.
...
We encounter some individual dogs, note how many of them were friendly ($d$), and form a posterior belief distribution about the friendliness of dogs.
...
What if we wanted to communicate this belief distribution?
...
A simple meaning then would be a threshold on this posterior probability: $\denote{u_{generalization}}(h, \theta): \{ h: h > \theta\}$.
$\denote{u_{generalization}}$ represents the semantic meaning of a statement conveying a generalization.
...
Unfortunately, no fixed values of $\theta$ would suffice: *Bills writes poems* might true if Bill does it a few times a year, while we might be hesitant to say *Mary smokes cigarettes* if she only does it once every few months.
...
For the case of our semantics for generalizations, these are degrees of propensity $h$ that are greater than the semantic threshold $\theta$.
The threshold $\theta$ is assumed to be drawn by an uninformative prior distribution, defined on the same scale as the propensity $h$: $\theta \sim \text{Uniform}(0, 1)$.
Finally, the prior distribution $P(h)$ is a distribution over the degrees of propensity. 
...
\begin{equation} 
S_{1}(u \mid k) \propto \exp{(\lambda \cdot U(u; k)  }
\label{eq:S1}
\end{equation}
...
\begin{equation}
U(u; h) = \ln{ \int_{\theta} L_0(h, \theta \mid u)}  \diff \theta - C(u)
\label{eq:utility}
\end{equation}
...
Because we intend $S_1$ to be a model of felicity judgments, we consider only the possibility of staying quiet in addition to saying the generalization as utterances [@Degen2014]. 
...
We assume no utterance cost between these two alternatives: $C(u_{generalization}) = C(u_{silence})$.
We implemented this model in the probabilisitic programming language WebPPL [@dippl], and a complete, runable version of the modelcan be found at \url{http://forestdb.org/models/generics.html}

## Model simulations

This model makes the prediction that the interpretation of a generalizatoin (and hence, its corresponding truth judgment) depends on the prior on propensity $P(h)$.
We first explore the model qualitatively by running simulations of $L_0$ and $S_1$ under various schematic priors.
We also compare this to two models with the same prior knowledge but where $\theta$ is fixed to either 0 or 0.5 (corresponding intuitively to *some* or *most*).

Before describing the results of the simulations, consider how an uniformly underspecified threshold would interact with a uniformly uncertain belief about a probability or propensity.
When the threshold is very high, only the highest probabilities would pass the threshold.
When the threshold is slightly lower but still high, the highest probabilities would still pass the threshold, as would some that are slightly lower.
As the threshold takes on lower and lower values, more and more probabilities would surpass it.
Taken together, the higher the probability, the more thresholds it passes.


When the threshold is at 0.5, everything greater than 
...
Figure \ref{fig:tjScatters} (left) shows the correspondence between the frequency of the event (transformed a log times-per-year scale) and the felicity of the corresponding habitual sentence. 
It is clear that a habitual sentence can receive strong agreement even when the actions are very infrequent (log frequency $\sim$ -1; 3 times in a 5-year interval; e.g. \emph{writes novels}, \emph{climbs mountains}).
...
Generalizations in language (e.g., *John runs.*, *Dogs are friendly.*, *The block makes the machine play music*) are a premier example of how simple statements --- statements understood by even the youngest language users --- can appear to have complex meanings.
...
Indeed we have been able to capture the context-sensivity of these statements by positing a single underlying scale defined by the *propensity* or probability.
We have shown that listeners' prior knowledge about this scale is causally related to their endorsement of statements of genericity.
Finally, our framework naturally accounts for a number of well-documented phenomena from cognitive and developmental psychology concerning *generic language*, or generalizations about categories.
This paper provides a theoretical framework for asking further questions about generalizations in language, which we outline below. 

## The Comparison Class and the Question Under Discussion

In this paper, we proposed a model for understanding generalizations in language that relies upon interlocutors' shared beliefs over the statistics of the event or property.
We constructed the prior belief distribution for the propensity of an action, cause, or property by considering other possible people of doing the action, other possible causes of producing the effect, or other possible categories of having the property.
...
We have shown similar sensitivity to the statistics of comparison class in the language of generalizations, thus incorporating generic, habitual, and causal language into the study of *vague language*. 

We made the design decision in this work to construct the comparison class with respect to other people, causes, or kinds.
In this way, the statement of genericity is addressing the implicit Question Under Discussions (QUDs): *who \textsc{does action}?*, *what \textsc{causes effect}?*, or *what \textsc{has feature}?*, respectively.
There is another clear way to construct the comparison class: With respect to other features. 
This would correspond to addressing the implicit QUDs: *what does \textsc{person} do?*, *what \textsc{effects} does \textsc{cause} bring about?*, and *what features does \textsc{kind} have?*

It's clear that statements can be used to address either QUD, and this difference can even be observed the same sentence.
"Lawyers care about the law." could be interpreted with a category-wise comparison class (i.e., *Lawyers (as opposed to doctors, firefighters, ...) care about the law.*; say, in a pedagogical context) or a feature-wise comparison class (i.e., *Lawyers care about the law (as opposed to justice, ethics, ...)* e.g., when said sardonically).
We chose to focus in this work on the category-wise comparison class for methodological convenience. 
In our three case studies, the category-wise comparison classes are fixed to *other people*, *other causes*, or *other kinds*.
Future work should address the property-wise reading of generalizations, and explore the cues that lead listeners to one or the other interpretation.
...
KT saw that people don't do well reasoning about explicit probabilities, and concluded that people are bad with probabilities.

But what if people are good, and just the way of conveying them is more vague?

We can communicate 75% (assuming the priors are consistent with that), and we can communicate 30% (when the priors are consistent).

So people do show sensitivity to these probabilities, suggesting that people are good with probabilities (CITE developmental work).
...
-- Sterken (2015): Quantificational force is like the uncertain threshold
-- Cohen (1999): Similar machinery (e.g., Alt(K)); don't rely on domain restriction necessarily because it opens up a number of theoretical doors that require more theorizing .  THat being said, there may be a formulation of the uncertain threhsold model that uses a fixed threshold with domain restriction uncertainty. 
...
-- Books are paperbacks (contrast class)
-- Birds lay eggs and are female vs. Elephants live in Africa and Asia
-- Mary handles the mail from antarctica (predictive propensity; also highlights domain restriction on events)
-- Women are submissive. (Haslanger, 2011)
